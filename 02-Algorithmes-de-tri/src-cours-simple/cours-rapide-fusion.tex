...
\begin{frame}[fragile]{La partition}
\begin{lstlisting}
def partition(t, g, d):
    """Partitionne t[g..d] autour du pivot t[d] ; renvoie la place du pivot."""
    p = t[d]
    i = g                      # t[g..i-1] contient les elements <= p
    for j in range(g, d):
        if t[j] <= p:
            t[i], t[j] = t[j], t[i]
            i += 1
    t[i], t[d] = t[d], t[i]    # on place enfin le pivot
    return i
\end{lstlisting}
\begin{encadre}\small
Un seul parcours du tableau, avec une comparaison par tour :
la partition de $n$ éléments coûte $n-1$ comparaisons, donc $\Th(n)$,
et n'utilise \textbf{aucune mémoire supplémentaire}.
\end{encadre}
\end{frame}
...
def tri_rapide(t, g=0, d=None):
    if d is None:
        d = len(t) - 1
    if g < d:                    # cas de base : 0 ou 1 element
        k = partition(t, g, d)   # diviser
        tri_rapide(t, g, k - 1)  # regner a gauche
        tri_rapide(t, k + 1, d)  # regner a droite
    return t
\end{lstlisting}
\vspace{2pt}
\begin{terminaisonrec}
Le pivot est \emph{exclu} des deux appels récursifs : la taille de la tranche
$d-g+1$ décroît donc strictement à chaque appel.
\end{terminaisonrec}
\begin{correction}[Correction (récurrence sur la taille de la tranche)]
\small
Après la partition, $t[k]$ est à sa place et tout élément à sa gauche lui est
inférieur ou égal, tout élément à sa droite lui est supérieur. Si les deux tranches
sont triées, le tout est trié.
\end{correction}
\end{frame}

\begin{frame}{Complexité : tout dépend du pivot}
...
C'est ce qui arrive si le tableau est \emph{déjà trié} : une tranche vide, une tranche
de taille $n-1$.
\[
...
\vspace{4pt}
\begin{encadre}
\textbf{1. Pivot aléatoire} --- tirer au hasard l'indice du pivot puis l'échanger avec
\tab{t[d]}. Aucune entrée particulière n'est plus « mauvaise » qu'une autre : le
$\Th(n\log n)$ devient vrai \emph{en moyenne pour toute entrée}.
\end{encadre}
\vspace{3pt}
\begin{encadre}
\textbf{2. Médiane de trois} --- prendre la médiane de $t[g]$, $t[(g+d)//2]$ et $t[d]$.
Élimine le pire cas « tableau déjà trié », de loin le plus fréquent en pratique.
\end{encadre}
\vspace{3pt}
\begin{encadre}
\textbf{3. Bascule} --- pour une tranche de moins de $15$ éléments, terminer par un
...
Mémoire auxiliaire & $\Th(\log n)$ (pile d'appels) : \textbf{tri en place}\\
Stabilité & \textbf{non stable} : les échanges se font à distance\\
\bottomrule
\end{tabular}
\end{center}
\vspace{8pt}
...
C'est pourtant le plus rapide en pratique, car il travaille en place,
sans recopie de tableau.
\end{attention}
\end{frame}
...
\begin{enumerate}\setlength\itemsep{3pt}
  ...
  \item Le \motcle{tri rapide} est \emph{en place} et le plus rapide en pratique,
        mais il est en $\Th(n^2)$ dans le pire cas et il n'est pas stable.
